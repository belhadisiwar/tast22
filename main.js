...
function affiche2() {
  ...
  for (var i = 0; i < result.data.length; i++) {
    html += ' <div class="card">'
    html += '<img class="card-img-top img-fluid" src="http://'+result.data[i].imagePath+' " alt="Card image cap">'
    html += '<div class="card-body">'
    html += '<h4 class="card-title">'+result.data[i].label+'</h4>'
    html += '<p class="card-text">'+result.data[i].description+'</p>'
   // html += '<p class="card-text">'+result.data[i].id+'</p>'
    html += '</div>'
    html += '</div>'
    html += '</div>'
    document.getElementById("categ").innerHTML+=html;
